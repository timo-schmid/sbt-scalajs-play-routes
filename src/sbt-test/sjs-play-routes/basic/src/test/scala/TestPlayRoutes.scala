...
object TestPlayRoutes extends TestSuite {
  ...
  val tests = this {
    ...
    'root {
      Blog.index().map(checkHttpResponse(200))
    }

    'posts {
      Blog.posts().map(checkHttpResponse(200))
    }

    'defaultParam {
      Blog.defaultParam().map(checkHttpResponse(200))
    }

    'post1 {
      Blog.post(1).map(checkHttpResponse(200))
    }

    'addPost {
      Blog.addPost().map(checkHttpResponse(201))
    }

    'editPost {
      Blog.editPost(1).map(checkHttpResponse(200))
    }

    'editPost {
      Blog.deletePost(1).map(checkHttpResponse(200))
    }

    'post1comments {
      Blog.comments(1).map(checkHttpResponse(200))
    }

    'albums {
      Website.page("foobar").map(checkHttpResponse(200))
    }

    'users {
      Website.page("users").map(checkHttpResponse(200))
    }
